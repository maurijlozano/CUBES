...
ls -d */ | while read F; do 
    ...
    if [[ ! -f "./${F}/"Tai_vs_DCBS_GNM2.svg || ! -f "./${F}/"tAi_modal_GNM2.svg ]] ; then
        echo 'You need to run tAi analysis first.'
        continue
    fi
    
    if [[ ! -f "./${F}/"tAi_modal_woPHE_GNM2.svg ]] ; then
                echo 'You need woPHE modal frequences.'
    svg_stack.py --direction=h --margin=30 "./${F}/"Tai_vs_DCBS_GNM2.svg "./${F}/"tAi_modal_GNM2.svg > "${FN}"3.svg
        continue
    fi

    svg_stack.py --direction=h --margin=30 "./${F}/"Tai_vs_DCBS_GNM2.svg "./${F}/"tAi_modal_woPHE_GNM2.svg > "${FN}"3.svg
done
