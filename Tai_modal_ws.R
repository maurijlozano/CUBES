...
##Calculate tAI vs dist on R incorporating singleton genes
...
sopt_DCBS <- read.table("s_opts_DCBS_ws.txt", header=TRUE, sep="\t", dec=".")
...
svg(filename="tAi_modal_ws.svg", width=8, height=6, pointsize=10)
...
if (length(fdata.woPHE$SET) > 1){
    ...
    svg(filename="tAi_modal_woPHE_ws.svg", width=8, height=6, pointsize=10)
