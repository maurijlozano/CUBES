...
modal <- data[ grep("C.{1,2}$|^PHE",data$class), ]
woPHE <- data[ grep("woPHE",data$class), ]

varpc = read.table("coa_var_axis1_2_C.txt", header=TRUE, sep=" ") #check field separator
varpc <- varpc*100
xlabtext <- paste("C1 (",varpc[1,1]," %)", sep="")
ylabtext <- paste("C2 (",varpc[1,2]," %)", sep="")


svg(filename="CA_genes_C.svg", width=5, height=4, pointsize=10)

ggplot (data, aes( y = Axis2, x = Axis1)) +
geom_point(data = genes, color='gray', size = 1, alpha = I(0.3)) +
geom_point(data = modal, fill = "dodgerblue3", size=3, colour="black", shape=21, stroke = 1) +
...
svg(filename="CA_genes2_C.svg", width=5, height=4, pointsize=10)
...
if (length(woPHE$class) != 0) {
    ...
    svg(filename="CA_genes_C_woPHE.svg", width=5, height=4, pointsize=10)
...
theme(axis.text.x = element_text(size=14),
    ...
    svg(filename="CA_genes2_C_woPHE.svg", width=5, height=4, pointsize=10)
