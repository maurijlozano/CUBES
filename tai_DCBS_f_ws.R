...
dcbs <- read.table("DCBS_ws.txt", header=T, row.names = 1, sep=" ")
dcbs <- dcbs[,c(65)]

hdata <- read.table("../freq_head.txt", header=FALSE, sep=" ")
f.counts <- read.table("freq_ws.counts", header=FALSE, row.names = 1, sep=" ")
...
write.table(TS_table, file = "s_opts_DCBS_ws.txt", sep = "\t", dec = ".", row.names = TRUE)
write.table(data, file = "tai_vs_DCBS_ws.txt", sep = "\t", dec = ".", row.names = TRUE)
...
svg(filename="Tai_vs_DCBS_ws.svg", width=8, height=6, pointsize=10)
