...
ls -d */ | while read F; do 
    ...
    if [[ ! -f "./${F}/"CA_genes_GNM.svg || ! -f "./${F}/"CA_genes2_GNM.svg || ! -f "./${F}/"CA_codons_GNM.svg ]] ; then
        echo 'You need to run coa.sh for CA analysis first.'
        continue
    fi

    svg_stack.py --direction=h --margin=30 "./${F}/"CA_genes_GNM.svg "./${F}/"CA_codons_GNM.svg  > "${FN}"_fig1.svg
