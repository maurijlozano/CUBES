...
fdataPHE <- fdata[ grep("^PHE",rownames(fdata)), ]
fdataSINGLE <- fdata[ grep("[Ss]ingle",rownames(fdata)), ]

fdata1 <- data.frame(rbind(fdata1,fdataPHE,fdataSINGLE))
...
Dist <- as.data.frame(rbind(Dist, c("PHE",Dist[phedist-1,2]+Dist[phedist-1,2]/10),c("Singletons",Dist[1,2]-Dist[phedist-1,2]/10)))
Dist$Dist <- as.numeric(Dist$Dist)
...
for (i in aminoacids) {
	...
	vlineint0 <- (0 + (Dist[1,2]-Dist[phedist-1,2]/10)/3)
	vv <- NULL
	tt <- NULL
...
#	myplot$Codon <- paste(myplot$Codon,"[",tt,"]","(",round(vv,2),")", sep="")	
	...
	p1 <- ggplot(myplot, aes(x=X,y=Freq, group=Codon, color=Codon)) +
		...
		geom_line(aes(color=Codon), size=1, show.legend=F)+
		...
		geom_vline(xintercept = vlineint0 , linetype="dashed", color = "black", size=0.5) +
		geom_dl(aes(label = Codon), method = list('first.bumpup', dl.trans(x = x +2.5, y = y ),cex=0.9)) +
...
a <- ggarrange(pC,pD,pE,pF,pH,pK,pN,pQ,pY,pI,pA,pG,pP,pT,pV,pL,pR,pS, ncol = 18, align = "hv",labels = aminoacids, hjust = -2.5, vjust = 1.5, font.label = list(size = 20, color = "black", face = "bold"))
b <- ggarrange(ptC,ptD,ptE,ptF,ptH,ptK,ptN,ptQ,ptY,ptI,ptA,ptG,ptP,ptT,ptV,ptL,ptR,ptS, ncol = 18, align = "hv",hjust = -2.5, vjust = 2)

name2 <- paste(name,"AAfreq_trnas_wi_F3.svg",sep="_")
