...
ls -d */ | while read F; do 
    ...
    if [[ ! -f "./${F}/"CA_genes_ws.svg || ! -f "./${F}/"CA_genes2_ws.svg || ! -f "./${F}/"CA_codons_ws.svg ]] ; then
        ...
        if [[ ! -f "./${F}/"IvsD_modal.svg || ! -f "./${F}/"GC3vsD_modal.svg ]] ; then
               echo 'You need to run Index_modal.sh | GC3_modal.sh first.'
        svg_stack.py --direction=h --margin=30 "./${F}/"CA_genes_ws.svg "./${F}/"CA_genes2_ws.svg "./${F}/"CA_codons_ws.svg  > "${FN}".svg
               continue
        fi
    fi

    svg_stack.py --direction=h --margin=30 "./${F}/"CA_genes_ws.svg "./${F}/"CA_genes2_ws.svg "./${F}/"CA_codons_ws.svg  > 1.svg
    ...
    if [[ -f "./${F}/"CA_genes2_ws_woPHE.svg && -f "./${F}/"IvsD_modal_woPHE.svg ]] ; then
    svg_stack.py --direction=h --margin=30 "./${F}/"CA_genes_ws.svg "./${F}/"CA_genes2_ws.svg "./${F}/"CA_genes2_ws_woPHE.svg "./${F}/"CA_codons_ws.svg  > 3.svg
