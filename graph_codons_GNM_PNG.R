...
png(filename = "CA_codons_GNM.png", width = 1000, height = 800, units = "px", pointsize = 12,
    bg = "white", res = 230, type = "cairo-png")

ggplot (data, aes( y = Axis2, x = Axis1)) +
geom_point(fill = "grey65", size=2, colour="black", shape=21, stroke = 0.5) +
geom_point(data=dataU, fill = "#61b8ffff", size=2, colour="black", shape=21, stroke = 0.5) +
geom_point(data=dataC, fill = "#8a0000ff", size=2, colour="black", shape=21, stroke = 0.5) +
xlab(xlabtext) + ylab(ylabtext) + labs(fill = "Legend")  +
geom_text_repel(aes(label=label, fontface=2), size = 2.5, point.padding = 1 , arrow = arrow(length = unit(0.01, 'npc'))) +
theme_calc() +
