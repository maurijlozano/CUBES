...
#Calculate tAI vs dist on R using Sij tuller, and incorporating singleton genes
...
Single$tai_t <- as.numeric(as.character(Single$tai_t))
fdata$tai_t <- as.numeric(as.character(fdata$tai_t))
C$tai_t <- as.numeric(as.character(C$tai_t))


yminimo <- min(fdata$tai_t,PHE$tai_t[1] ) - (.1 * (max(fdata$tai_t,PHE$tai_t[1]) -min(fdata$tai_t,PHE$tai_t[1])))
ymaximo <- max(fdata$tai_t,PHE$tai_t[1]) + (.1 * (max(fdata$tai_t,PHE$tai_t[1]) -min(fdata$tai_t,PHE$tai_t[1])))

svg(filename="tAi_modal_ws_t.svg", width=8, height=6, pointsize=10)
...
annotate("text", min(Dist$Dist), Single$tai_t[1], vjust = 1.5, hjust = -0.1, label = "Singletons", size=6) +
geom_hline(yintercept = Single$tai_t[1]  , linetype="dashed", color = "Blue", size=1) +
...
if (length(fdata.woPHE$SET) > 1){
    ...
    svg(filename="tAi_modal_woPHE_ws_t.svg", width=8, height=6, pointsize=10)
...
Single$tai_dr <- as.numeric(as.character(Single$tai_dr))
fdata$tai_dr <- as.numeric(as.character(fdata$tai_dr))
C$tai_dr <- as.numeric(as.character(C$tai_dr))


yminimo <- min(fdata$tai_dr,PHE$tai_dr[1] ) - (.1 * (max(fdata$tai_dr,PHE$tai_dr[1]) -min(fdata$tai_dr,PHE$tai_dr[1])))
ymaximo <- max(fdata$tai_dr,PHE$tai_dr[1]) + (.1 * (max(fdata$tai_dr,PHE$tai_dr[1]) -min(fdata$tai_dr,PHE$tai_dr[1])))

svg(filename="tAi_modal_ws_dr.svg", width=8, height=6, pointsize=10)
...
annotate("text", min(Dist$Dist), Single$tai_dr[1], vjust = 1.5, hjust = -0.1, label = "Singletons", size=6) +
geom_hline(yintercept = Single$tai_dr[1]  , linetype="dashed", color = "Blue", size=1) +
geom_text_repel(aes(label=SET, fontface=2), size = 6, point.padding = 1 , show.legend = FALSE, arrow = arrow(length = unit(0.01, 'npc'))) +
scale_color_manual(values=c('black'))+
ylim(yminimo, ymaximo) +
xlab("Evolutionary distance") +
...
if (length(fdata.woPHE$SET) > 1){
    ...
    svg(filename="tAi_modal_woPHE_ws_dr.svg", width=8, height=6, pointsize=10)
