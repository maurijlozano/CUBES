...
if (length(args)==0) {
  stop("At least one argument must be supplied (Iterations).n", call.=FALSE)
} 
...
dcbs <- read.table("DCBS.txt", header=T, row.names = 1, sep=" ")
dcbs <- dcbs[,c(65)]
...
s.values = foreach(i=1:args[1], .export=c('tai_DCBS_corr'), .packages='tAI', .combine=merge_data) %dopar% {
    sv <- runif(5, 0, 1)
...
write.table(TS_table, file = "s_opts_DCBS.txt", sep = "\t", dec = ".", row.names = TRUE)
write.table(data, file = "tai_vs_DCBS.txt", sep = "\t", dec = ".", row.names = TRUE)
...
svg(filename="Tai_vs_DCBS.svg", width=8, height=6, pointsize=10)
