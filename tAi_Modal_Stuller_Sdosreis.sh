...
{
echo "
        Usage: tAI_Modal_Stuller_Sdosreis.sh [-y] [-h]

        -y Skips command confirmation
        -h Show this Help
"

echo -e '\nComputes tAi Adaptaion Index of Modal frequencies.\nThis script scans all the child directories for C1*.modal_freq files and computes de adaptation index tAi using Svalues from Sabi et. al  and DosReis et.al papers.\nGenerates a tAi vs Evolutionary distance plot.\n\n'
exit 1
}
...
ls -d */ | while read F; do 
    ...
    if [[ -f modal.counts ]] ; then
        ...
        cd ..
        Rscript --vanilla Tai_modal2.R "./${F}"
        continue
    fi
